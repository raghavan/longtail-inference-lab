...
The cloud teacher solves only preregistered public memory-build tasks. Raw artifacts stay in ignored local storage; strict local sanitizer, Gitleaks, canary, contamination, and residual gates run before an allowlisted sanitized-evidence packet may be sent to the same model for distillation. External human approval scoped to exact hashes is mandatory before admission. The exact local Qwen student is then evaluated under paired M0 (no memory) and M2 (approved Markdown retrieval), with the retrieved-memory block as the only student-context difference.
