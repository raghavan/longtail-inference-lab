# GPT-5.6-sol teacher / local Qwen 32K transfer preregistration

**Status:** Frozen protocol awaiting merge; no measured execution authorized before this preregistration lands  
...
**Machine-readable freeze:** [`manifests/preregistration-freeze-2026-08-01.v1.json`](../manifests/preregistration-freeze-2026-08-01.v1.json)

This document is the immutable preregistration for the first real cloud-teacher/local-student Terminal Artifact Memory experiment. The landed merge revision, not this feature-branch revision, becomes the measured code pin. Any change to a task, role, model, adapter, prompt, context policy, task pin, qualification hash, ordering, checkpoint, threshold, denominator, or stop rule requires a new preregistration and a fresh attempt budget.
...
No measured teacher build, cloud distillation, Qwen M0, Qwen M2, memory admission, or efficacy analysis has run. The only pre-freeze executions are development controls:
...
4. a non-measured credential/isolation negative control.
...
1. this preregistration PR to be merged;
...
1. Merge this preregistration and pin the exact landed clean revision.
...
1. this PR is not merged or the checkout is not the exact clean landed revision;
...
- Docker client `24.0.4`, server `28.3.2`, API `1.51`, Compose `2.39.1-desktop.1`.
