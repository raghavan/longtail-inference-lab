...
**Status:** Native Mac voice input, text answers, and optional local read-aloud work; the owner confirmed basic iOS TestFlight operation. The [Jetson order record](02_edge_offline_intelligence_device/hardware/portable_procurement.md) documents the first mains-powered voice prototype's parts. All eight first-batch items are ordered; arrival, setup, and device acceptance remain open. Model-selection intake remains open. Zero published comparative quality or performance measurements for Mac, iPhone, or Jetson.
