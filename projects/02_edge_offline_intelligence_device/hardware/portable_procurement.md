...
Apple's recognition, answer model, and installed Premium voices cannot be copied to Jetson. Preserve the interaction contract, prompt policy, cancellation, and evaluation cases; implement Linux audio/UI and local open-model backends. Start evaluation within roughly the 1–2B quantized answer-model band already listed in the [architecture](../design_direction.md), with a compact English transcriber and local speech synthesizer. Exact weights, quantization, runtime revision, context, and voice must be selected and recorded before measured runs. Naturalness is a listening result, not a property established by buying a speaker or GPU.
