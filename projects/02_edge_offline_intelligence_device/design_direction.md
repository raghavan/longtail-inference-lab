...
Start Jetson trials around a 1–3B parameter answer model with 4-bit weights, a compact English transcriber, and local speech synthesis. Exact artifacts, runtimes, context limits, voice quality, and memory headroom remain to be qualified. The Apple backend and installed voices cannot be transferred to Jetson. Screen, battery, and portable enclosure are deferred; approximately two hours per charge remains a future unmeasured target. Read the [order record and arrival checklist](hardware/portable_procurement.md) before setup or implementation. No Jetson code or measurement is established by this design record.
