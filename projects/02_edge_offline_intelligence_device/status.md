...
Hardware purchasing resumed. The owner confirmed placing the six-item DigiKey accessory order on September 13. The item export matches the microphone, speaker, button, reader, and two data cables at one each, totaling $48.30 in parts. The final delivered total and fulfillment status remain pending; do not reorder those six items. The [shopping list and restart checklist](hardware/portable_procurement.md) records the remaining complete Orin Nano Super 8 GB kit from Arrow and exact SanDisk Extreme 128 GB card from Newegg; successful kit and card orders are not yet confirmed. Recheck their current price, availability, and order status before any further checkout. The first hardware stage is mains-powered speech input and output with a physical listening button; screen, battery, and portable enclosure are deferred. The [budget](hardware/bill_of_materials.md) preserves the $1,000 first-year ceiling and separates the known ordered parts value from unconfirmed charges and planning allowances. Jetson software, exact models, voice quality, memory headroom, latency, and the future battery target remain unmeasured.
