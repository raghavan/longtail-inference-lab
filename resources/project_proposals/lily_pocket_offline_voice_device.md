# Lily Pocket: Offline Voice Intelligence Device
...
The working name is **Lily Pocket**.
...
Lily Pocket explores the opposite design:
...
This idea is broader than one experiment because it requires several hardware and software capabilities to behave as one dependable appliance:
...
The enclosure should hide the development board, cables, storage, audio module, and battery system.
...
Push to talk provides several advantages:

1. Lower idle power consumption
2. Stronger privacy
3. Fewer accidental activations
4. No need for continuous speech processing
...
The device can stop recording before it begins playback. This half duplex interaction avoids the need for a large microphone array and complex acoustic echo cancellation.
...
The project should not assume that the more expensive module is necessary. It should first measure whether the 8 GB prototype delivers an acceptable experience.

### Storage

Use a local NVMe SSD rather than relying on removable flash storage for routine use.
...
The final device should not expose the storage during normal use.
...
This module is appropriate because Lily Pocket is a close range personal assistant rather than a room scale smart speaker.
...
The project should move to the larger audio board only when measurements show that the tiny module is not usable.
...
The device should support two operating modes:
...
6. Replaceable or serviceable battery pack when possible
...
4. No visible branding beyond a subtle Lily mark
...
Begin with Piper because it is compact and already represented in the Jetson container ecosystem.
...
The system should report actual peak memory rather than assuming that individual model file sizes represent runtime use.

## Offline behavior

Offline operation is a hard requirement rather than a marketing label.

The project should verify operation with:
...
Any feature that silently falls back to the cloud is out of scope for the trusted path.
...
1. Off: idle or powered down
2. Soft white: ready
3. Gentle pulse: listening
4. Slow pulse: thinking
5. Steady warm light: speaking
6. Red: physical microphone mute or error
7. Brief amber pulse: low battery
...
5. Larger audio module only when required
...
Longtail Inference Laboratory studies how useful intelligence can move closer to the person and operate across ordinary devices rather than remain dependent on distant data centers.

Lily Pocket turns that thesis into a physical object.
