...
**Status:** Specified — active focus; laptop prototype exists, zero published device measurements
...
**Current evidence:** no published device measurements. The laptop controller and local app have documented dry-run and stub checks; those checks do not establish real-model or Jetson performance. No hardware purchase is recorded in the project. Every device number is a target or a budget until a results file says otherwise.
...
## Active work decision — September 12, 2026

The laboratory is directing its active work toward building this device. The existing charter and Experiment 02.1 remain the agreed design; the hardware tier, $500 ceiling, push-to-talk interaction, and laptop-first build order carry forward.

The immediate gate is to validate the complete laptop loop with real speech recognition, answer generation, and speech synthesis, and record the outcome with the pilot's measurement limits. The [local app](software/app/README.md) is available as an interaction rehearsal. Its stub checks and interface traces are development evidence.

Before measured Jetson runs, freeze the exact model files, revisions and hashes, runtime versions, context and decoding controls, JetPack version, power mode, and reference audio configuration in the existing experiment record. Battery and enclosure work retain their later measurement gates.

## Decisions already taken

| Decision | Choice | Date | Basis |
| --- | --- | --- | --- |
| Compute tier | Jetson Orin Nano Super 8 GB | August 9 2026 | Hold initial hardware spend under $500 and measure the 8 GB residency constraint |
...
The 8 GB limit makes model residency an empirical question: **do three models fit at once, and if they do, is residency worth its complexity?** The initial build stays within the agreed $500 hardware ceiling. Any larger tier requires evidence from this device and a separate budget decision.
...
This project studies how much useful intelligence can be carried, owned, and relied upon when nothing else is reachable. The complete device defines the scope of the claim. Fixed controls, named measurements, and visible failure cases determine which capabilities the evidence supports.
